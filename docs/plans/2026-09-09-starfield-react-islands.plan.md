...
```bash
diff <(git show main:index.html | grep -i 'content-security-policy') \n     <(git show HEAD:index.html | grep -i 'content-security-policy')
```

Expected: **nenhuma saída**, ou seja, as duas linhas de CSP são idênticas. Qualquer diferença é falha de aceite.

Não usar `git diff main -- index.html | grep security-policy`: o `grep` sobre um `diff` casa também com linha de contexto, então ele acusa mudança onde não houve. O `diff` entre as duas versões da linha é o teste certo.
